...
void Init_SPI3_Master(void)
{
    
    TRISCbits.TRISC0  = 0;  // SDO3 output
    TRISDbits.TRISD5  = 1;  // SDI3 input
    TRISBbits.TRISB10 = 0;  // SCK3 output (master)
    TRISBbits.TRISB6  = 0;  // SS manual output
    
    
    SPI3CONbits.ON = 0;     // Disable SPI module before configuration
    
    uint32_t dummy = SPI3BUF; // Clear receive buffer (flush FIFO)
    dummy = SPI3BUF;
    (void)dummy;
    
    SPI3STATbits.SPIROV = 0;    // Clear overflow flag
    //Set baud rate
    // SPI clock = Fpb / (2 * (SPI3BRG + 1))
    // For Fpb = 60 MHz, SPI = 1 MHz:
    SPI3BRG = (60000000 / (2 * 500000)) - 1;   // => 19
    // Configure Master mode and clock settings
    SPI3CONbits.MSTEN = 1;   // Master mode
    SPI3CONbits.CKP = 0;     // Clock idle low
    SPI3CONbits.CKE = 0;     // Data changes on falling edge
    SPI3CONbits.SMP = 0;     // Input data sampled at end of output time
    SPI3CONbits.ENHBUF = 0;  // Disable enhanced buffer (simple mode)
    // 8-bit mode
    SPI3CONbits.MODE16 = 0;
    SPI3CONbits.MODE32 = 0;
    
    SPI3CONbits.FRMPOL = 0; //  Frame Sync Polarity bit
    SPI3CONbits.MSSEN = 1;  // Master Mode Slave Select Enable bit
