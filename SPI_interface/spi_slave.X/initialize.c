...
void Init_SPI3_Slave(void)
{
    ...
    TRISCbits.TRISC0  = 0;  // SDO3 (input when in slave mode)
    TRISDbits.TRISD5  = 1;  // SDI3 output
    TRISBbits.TRISB10 = 1;  // SCK3 input
    TRISBbits.TRISB6  = 1;  // SS input
    
    SPI3CONbits.ON = 0;     // Disable SPI before config
    uint32_t dummy = SPI3BUF; // Clear buffer and overflow flag
    (void)dummy;
    ...
    SPI3CONbits.SSEN = 1;
    SPI3CONbits.CKP = 0;     // Idle clock low
    SPI3CONbits.CKE = 0;     // Data changes on falling edge
    SPI3CONbits.SMP = 0;     // Sample in middle of data
    SPI3CONbits.ENHBUF = 0;  // No enhanced buffering
    ...
    // Set priority/subpriority (3:2 is fine)
    IPC54bits.SPI3RXIP = 3;
    IPC54bits.SPI3RXIS = 2;

    IEC6bits.SPI3RXIE = 1;  // Enable SPI3 RX interrupt
    SPI3CONbits.ON = 1;  // Enable SPI   
}
